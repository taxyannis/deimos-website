...
**Build status (current):** all seven required v1 routes are built and shipped — `/`, `/advisory`, `/firm`, `/jurisdictions`, `/contact`, `/legal` and `/privacy` (the last as a conservative, clearly-marked placeholder pending real counsel-drafted content). `/situations` and `/sectors` remain optional Phase 2.5.
